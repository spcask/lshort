...
% In addition to using the landscape option with the document class, it
% is also necessary to use the geometry package to set the PDF in
% landscape mode. Without it only the slides (content) are set in
% landscape mode but the PDF (white background) remains in potrait mode,
% and therefore the slides appear truncated on the right side.

% The default output dimension is 11 x 8.5 in, i.e., letter size in
% landscape mode.
...
% The "\tableofcontents" and "\section" commands lead to the following
% error while using the minimal class:
%
